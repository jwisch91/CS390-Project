...
public class Select_Workout_Days extends Activity{
	...
	public void addListenerOnSubmit(){
		...
		objCalendar.set(Calendar.WEEK_OF_MONTH, objCalendar.get(Calendar.WEEK_OF_MONTH) + 60*60*24*7); //sets it up starting the next week.  Gives users time to prepare
				  ...
				  if (mon.isChecked()){
						objCalendar.set(Calendar.DAY_OF_WEEK, 1); // Monday of the next week 
							...
							if(com.example.testing.Select_workout.loseWeight == true){
						        ...
						        calIntent.putExtra(Events.RRULE, "FREQ=WEEKLY;COUNT=12;WKST=SU;BYDAY=MO"); // every monday for the next 12 weeks
